...
if($search){
  $statement= $pdo->prepare('SELECT * FROM approvetable  WHERE fname LIKE :fname ORDER BY date DESC');
 $statement->bindValue(":fname","%$search%");


}else{
$statement= $pdo->prepare('SELECT * FROM approvetable ORDER BY date DESC');
}
$statement->execute();
